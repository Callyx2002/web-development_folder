...
Challenge 4
Write a function once that accepts a callback as input and returns a function. When the returned function is called the first time, it should call the callback and return that output. If it is called any additional times, instead of calling the callback again it will simply return the output value from the first time 
it was called.
...
// const toRun = function () { console.log("My name is Caleb") };
// delay(toRun, 4000);
...
function dateStamp(func) {
  function passedIn(param) {
    return { date: new Date().toLocaleTimeString(), output: func(param) };
  }
  return passedIn;
}
...
function censor() {
  ...
  function string(...str) {
    if (arguments.length === 2) {
      myArray.push([arguments[0], arguments[1]]);
    }
    if (arguments.length === 1) {
      let newStr = arguments[0];
      for (let [first, second] of myArray) {
        console.log(first, second);
        newStr = newStr.replace(first, second);
      }
      return newStr;
    }
  }
...
function makeFuncTester(arrOfTests) {
  function smaller(callback) {
    return callback(arrOfTests[0][0]) === arrOfTests[0][1];
  }
  return smaller;
}
